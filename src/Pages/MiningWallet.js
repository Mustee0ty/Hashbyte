  ...
  return (
    <section className='lg:w-[360px] lg:h-[980px] sm:w-max-screen sm:h-screen mx-auto bg-[#F8F7FE]' >
      ...
      <div className='w-[360px] mt-[35px] cursor-pointer relative mx-auto'>
        <div onClick={TradeMine} className='flex flex-row h-[37px] w-[138px] text-[#afafaf] bg-white rounded-2xl mx-auto'>
          <div className={` flex items-center justify-center ${isSelected? 'flex w-1/2 bg-grey-300 rounded-full bg-blue-500 transition-all duration-500' : 'flex w-1/2 bg-white rounded-full transition-all duration-500'}`}>
          Trading
          </div>
    
        <div onClick={TradeMine} className={` flex items-center justify-center ${isSelected? 'h-full w-1/2 bg-white rounded-full transition-all duration-500' : 'w-1/2 h-full bg-[#5B63E6] rounded-full transition-all duration-500  ' }`}>
          Mining
          </div>
        </div>
        </div>
            ...
            walletBalance.map((items) => {
              ...
              return (
                <div className='flex flex-col mx-auto text-[#f4f8ff]' key={id}>
                  <p className='h-[35px] w-[153px] text-xl font-extrabold leading-[35px] '>{balance}</p>
      ...
      <div className='flex flex-col items-center justify-center my-[14px] bg-white rounded-[10px] w-[323px] h-[65px] mx-auto '>
        {
          HomeData.map((time) => {
            ...
            return (
              <p className='text-[20px] font-bold '>{upComingPayTime} <br /> 
              <p className='text-[15px] text-[#303c5b] w-full flex items-center justify-center align-middle mx-auto font-normal text-[v#9192a1]'>Upcoming payment</p> 
              </p>
            )
          })
        }
